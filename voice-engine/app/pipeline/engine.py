...
class SpeechToSpeechEngine:
    ...
    async def _vad_worker(self):
        ...
        while self._running and self.session.is_active:
            ...
            transition = self.turn_manager.handle_speech_frame(vad_res.is_speech, frame_data=frame.data, frame_duration_ms=frame.duration_ms)
            ...
            if transition == "BARGE_IN":
                ...
                current_speech_audio.extend(self.turn_manager.barge_in_pre_buffer)
                ...
                # Filter out short line noise bursts under 100ms
                if len(speech_bytes) < 3200:  # < 100ms of 16kHz audio = micro noise pop
                    logger.info("Ignoring micro noise pop (<100ms)")
                ...
                if self.turn_manager.current_state in (TurnStateEnum.LISTENING, TurnStateEnum.LISTENING_AFTER_BARGE_IN, TurnStateEnum.IDLE):
                    current_speech_audio.extend(frame.data)
    ...
    async def _process_stt_turn(self, audio_bytes: bytes, turn_id: str, generation_id: str, token: CancellationToken):
        ...
        try:
            stt_lang = (self.session.preferred_language or self.session.language) if self.session.language_selection_complete else None
    ...
    async def _llm_worker(self):
        ...
        while self._running and self.session.is_active:
            ...
            if isinstance(packet, dict):
                ...
                user_text = packet
                turn_id = self.session.current_turn.turn_id
                gen_id = self.session.current_turn.generation_id
                token = self.session.current_turn.cancellation_token

            if token.is_cancelled or self.session.is_generation_cancelled(gen_id):
            ...
            turn.state = TurnStateEnum.PROCESSING
            now_ms = time.time() * 1000
            logger.info(f"[TURN {turn.turn_id}] llm_started={now_ms:.3f}")
            if self._current_metrics and self._current_metrics.turn_id == turn.turn_id:
            ...
            detected_lang = packet.get("detected_lang") if isinstance(packet, dict) else None
            ...
            if direct_ack:
                ...
                logger.info(
                    ...
                    f"generation_id={turn.generation_id}\n"
                ...
                await self.queues.tts_in_queue.put({"delta": direct_ack, "turn_id": turn.turn_id, "generation_id": turn.generation_id, "token": token})
                await self.queues.tts_in_queue.put({"delta": "__EOF__", "turn_id": turn.turn_id, "generation_id": turn.generation_id, "token": token})
                continue
            ...
            if fast_resp:
                ...
                await self.queues.tts_in_queue.put({"delta": fast_resp, "turn_id": turn.turn_id, "generation_id": turn.generation_id, "token": token})
                await self.queues.tts_in_queue.put({"delta": "__EOF__", "turn_id": turn.turn_id, "generation_id": turn.generation_id, "token": token})
                continue

            try:
                messages = await self.conversation_manager.assemble_llm_messages(self.session, user_text)
                
                if token.is_cancelled or self.session.is_generation_cancelled(turn.generation_id):
                    continue
                ...
                async for chunk in self.llm_provider.stream_chat(messages, cancellation_token=token):
                    ...
                    if chunk.delta:
                        if self.session.is_generation_cancelled(turn.generation_id):
                            break
                ...
                if not token.is_cancelled and not self.session.is_generation_cancelled(turn.generation_id):
    ...
    async def _tts_worker(self):
        ...
        while self._running and self.session.is_active:
            ...
            if isinstance(item, dict):
                ...
                initial_chunk = item
                item_turn_id = self.session.current_turn.turn_id
                item_gen_id = self.session.current_turn.generation_id
                token = self.session.current_turn.cancellation_token

            if token.is_cancelled or initial_chunk == "__EOF__":
                continue

            if item_turn_id != self.session.current_turn.turn_id or item_gen_id != self.session.current_turn.generation_id:
                continue

            turn = self.session.current_turn
            turn.state = TurnStateEnum.SPEAKING
            now_ms = time.time() * 1000
            logger.info(f"[TURN {turn.turn_id}] tts_started={now_ms:.3f}")
            if self._current_metrics and self._current_metrics.turn_id == turn.turn_id:
                ...
                while True:
                    try:
                        ...
                        if isinstance(nxt, dict):
                            ...
                            if nxt_token.is_cancelled or nxt_gen != item_gen_id or nxt_delta == "__EOF__":
                                break
                            yield nxt_delta
                        else:
                            if nxt == "__EOF__" or token.is_cancelled:
                                break
                            yield nxt
            ...
            try:
                ...
                async for audio_chunk in self.tts_provider.stream_synthesize(
                    ...
                    language_code=self.session.preferred_language or self.session.language or "en-IN",
                    cancellation_token=token
                ):
                    if token.is_cancelled or self.session.is_generation_cancelled(item_gen_id):
                    ...
                    if first_audio:
                        ...
                        if self._current_metrics and self._current_metrics.turn_id == turn.turn_id:
                            ...
                            logger.info(
                                f"[VOICE_LATENCY] turn_id={turn.turn_id} "
                    ...
                    # Emit audio output event with base64 payload if generation is uncancelled
                    if not token.is_cancelled and not self.session.is_generation_cancelled(item_gen_id):
                        b64_audio = AudioCodec.frame_to_base64(frame)
                        self._emit_event(SessionEvent(
                            event=EventType.AUDIO_OUTPUT,
                            session_id=self.session.session_id,
                ...
                if not token.is_cancelled and not self.session.is_generation_cancelled(item_gen_id):
                    ...
                    self.session.user_has_floor = True
                    new_turn = self.session.start_new_turn(reason="Response completed naturally")
